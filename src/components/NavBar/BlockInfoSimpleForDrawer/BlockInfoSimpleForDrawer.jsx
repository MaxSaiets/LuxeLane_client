...
const BlockInfoSimpleForDrawer = ({blockTitle, data, useAccordion = false, bgcolorForAccordion = "#fff", toggleDrawer}) => {
    ...
    const content = (
        <Box sx={{display: "flex", flexDirection: "column"}}>
            {data.map((item, index) => (
                <Link onClick={toggleDrawer(false)} href={item.href} underline="hover" rel="noreferrer" key={index} sx={{ paddingTop: index === 0 ? "0px" : "5px", paddingBottom: index === data.length - 1 ? "0px" : "5px", fontSize: 16, color: theme.palette.text.main, "&:hover": {color: "#656464"}}} >
                    {item.text}
                </Link>
            ))}
        </Box>
